...
All plots are interactive and should be played with. Change the scale / data with the dropdown menus, select the data to be displayed with the legend, and zoom-in with left-click and drag, drawing a square or a horizontal / vertical line. Explore the upper-right corner of each plot to go through other possibilities such as toogling spikes lines. 
...
As a little bonus, this gives some 
 section aims to summarize the main characteristics of the disease from the available data. 

<!-- violin plot death -->
<iframe width="100%" height="500" frameborder="0" scrolling="no" src="//plotly.com/~stephanefevrier/208.embed?showlink=false"></iframe>

To understand the data used for the violin plot, the 15 most represented countries are showed through a Sankey diagram, where the relationship to Wuhan and the gender of the people studied are given.
