...
In the first part, the concept of compartmental models is quickly presented through one of the most simple model: the SIR model. Then, the more advanced model used here is described. In the second part, the fitting of the model to real data is presented, along with its predictions. Finally, the third part aims to provide an adjusted model according to newly available data.
...
While the model predictions *look* good (they do, it's beautiful), several important points needs to be considered:

- The general approach is naive (compartments used, the ICU beds implementation)
- Multiple parameters need to be adjusted:
  - $$\gamma$$ and $$\delta$$ should be re-estimated, along with the probability of going to ICU and the probability of dying at ICU
  - $$R_0$$ after lockdown is set at 1.1 to emulate a specific scenario. Using more data from after the lockdown's end could allow to fit it realistically.
  - The outbreak shift could be set to an earlier date
  - the number of beds should vary through time and actually fit the real evolution. It was assumed to be non so significative here.

However, making the model was fun (writing this post was not), and seriously it really looks good. In the following part, up-to-date data will be added.
